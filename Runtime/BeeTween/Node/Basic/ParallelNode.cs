...
namespace DataKeeper.BeeTween
{
    ...
    public class ParallelNode : IBeeTweenNode
    {
        ...
        public async Awaitable ExecuteAsync(CancellationTokenSource cancellationToken)
        {
            ...
            {
                try { await node.ExecuteAsync(cancellationToken); }
                catch (Exception ex)
                {
                    if (Interlocked.Decrement(ref remaining) == 0) completionSource.SetException(ex);
                    return;
                }
                if (Interlocked.Decrement(ref remaining) == 0) completionSource.SetResult(null);
            }

            foreach (var node in Nodes) _ = ExecuteAndComplete(node);
