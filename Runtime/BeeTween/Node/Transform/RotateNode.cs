...
namespace DataKeeper.BeeTween
{
    ...
    public class RotateNode : IBeeTweenNode
    {
        ...
        public ITransformProvider TargetProvider { get; set; }

        [field: SerializeReference, SerializeReferenceSelector]
        public IVector3Provider TargetRotationProvider { get; set; }

        [field: SerializeReference, SerializeReferenceSelector]
        public IFloatProvider DurationProvider { get; set; }

        [field: SerializeReference, SerializeReferenceSelector]
        public IEaseProvider Ease { get; set; }

        public RotateNode()
        {
            TargetProvider = new TransformDirectProvider();
            TargetRotationProvider = new Vector3ConstantProvider();
        ...
        public async Awaitable ExecuteAsync(CancellationTokenSource cancellationToken)
        {
            ...
            var elapsedTime = 0f;
            var targetRotation = Quaternion.Euler(TargetRotationProvider.GetValue());
            ...
            while (elapsedTime < duration)
            {
                ...
                target.rotation = MathFunc.Lerp.LerpQuaternionUnclamped(startRotation, targetRotation,
                    easeProvider.Evaluate(Mathf.Clamp01(elapsedTime / duration)));
            }
